...
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Manage Students</title><link rel="stylesheet" href="../assets/css/style.css"></head><body><div class="admin-layout"><aside class="sidebar"><a class="brand" href="../index.php">Result <span>System</span></a><nav><a href="dashboard.php">Dashboard</a><a class="active" href="students.php">Students</a><a href="results.php">Results</a><a href="logout.php">Logout</a></nav></aside><main class="admin-main"><div class="page-heading"><p class="eyebrow">Administration</p><h1>Students</h1><p class="muted">Add and manage student records.</p></div><?php if($error):?><div class="alert error"><?=e($error)?></div><?php endif;?><?php if($success):?><div class="alert success"><?=e($success)?></div><?php endif;?><section class="panel"><div class="panel-head"><h2><?= $editStudent?'Edit Student':'Add Student' ?></h2><?php if($editStudent):?><a class="btn btn-ghost" href="students.php">Cancel</a><?php endif;?></div><form class="form-grid" method="post"><input type="hidden" name="action" value="save"><input type="hidden" name="id" value="<?=e((string)($editStudent['id']??''))?>"><label>Name<input name="name" required value="<?=e($editStudent['name']??'')?>"></label><label>Roll Number<input name="roll_number" required value="<?=e($editStudent['roll_number']??'')?>"></label><label>Email<input type="email" name="email" required value="<?=e($editStudent['email']??'')?>"></label><label>Course<input name="course" required value="<?=e($editStudent['course']??'BCA')?>"></label><label>Semester<select name="semester"><?php for($i=1;$i<=12;$i++):?><option value="<?=$i?>" <?=((int)($editStudent['semester']??4)===$i)?'selected':''?>><?=$i?></option><?php endfor;?></select></label><label>Password<input type="password" name="password" placeholder="<?=$editStudent?'Leave blank to keep current':'Default: student123'?>"></label><div class="form-actions"><button class="btn btn-primary" type="submit"><?= $editStudent?'Update Student':'Add Student' ?></button></div></form></section><section class="panel"><div class="panel-head"><h2>Student List</h2><form class="inline-search"><input name="q" value="<?=e($search)?>" placeholder="Search name or roll no"><button class="btn btn-secondary">Search</button></form></div><div class="table-wrap"><table><thead><tr><th>Roll No</th><th>Name</th><th>Course</th><th>Semester</th><th>Actions</th></tr></thead><tbody><?php foreach($students as $s):?><tr><td><?=e($s['roll_number'])?></td><td><?=e($s['name'])?></td><td><?=e($s['course'])?></td><td><?=e((string)$s['semester'])?></td><td class="actions"><a class="btn btn-small" href="?edit=<?=$s['id']?>">Edit</a><form method="post" onsubmit="return confirm('Delete this student and their result?');"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?=$s['id']?>"><button class="btn btn-small danger">Delete</button></form></td></tr><?php endforeach;?><?php if(!$students):?><tr><td colspan="5" class="empty-cell">No students found.</td></tr><?php endif;?></tbody></table></div></section></main></div></body></html>
